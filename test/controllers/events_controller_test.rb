...
class EventsControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_create_when_save_fails
    ...
    form = ev.attributes.except("id", "created_at", "updated_at")
    form["latitude"] = 100.0 # invalid

    assert_difference "Event.count", 0 do
      post events_url, :params => { :event => form.as_json }, :xhr => true
    end
  ...
  def test_update_as_organizer
    ...
    assert_equal I18n.t("events.update.success"), flash[:notice]
    e1.reload
  ...
  def test_update_put_failure
    ...
    assert_difference "Event.count", 0 do
      put event_url(ev), :params => { :event => form.as_json }, :xhr => true
    end
